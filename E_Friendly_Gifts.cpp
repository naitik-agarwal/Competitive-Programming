...
void solve() {
  ...
  v[0] = {1, {a[0], a[0]}};
  for (int i = 1; i < n; i++) {
    int lstart = v[i - 1].second.first;
    int lend = v[i - 1].second.second;
    int llen = v[i - 1].first;
    int current = a[i];
    if (current == lstart - 1) {
      v[i] = {llen + 1, {current, lend}};
    } else if (current == lend + 1) {
      v[i] = {llen + 1, {lstart, current}};
    } else {
      v[i] = {1, {current, current}};
    }
    pair<int, pair<int, int>> p1 = {
        v[i].first, {v[i].second.first - v[i].first, v[i].second.first - 1}};
    pair<int, pair<int, int>> p2 = {
        v[i].first, {v[i].second.second + 1, v[i].second.second + v[i].first}};
    for (int j = i - 1; j >= 0; j--) {
      if (v[j] == p1 || v[j] == p2) {
