...
void solve()
{
    ...
    //     }
    // }
    // cout << cnt / 2 << endl; // total number of greeting counted twice
    vector<pair<int, int>> hel;
    for (int i = 0; i < n; i++)
    {
        hel.push_back({b[i], a[i]});
    }
    sort(rall(hel));
    for (int i = 0; i < n; i++)
    {
        int temp = hel[i].first;
        hel[i].first = hel[i].second;
        hel[i].second = temp;
    }
    ordered_set<int> s;
    for (int i = 0; i < n; i++)
    {
        s.insert(hel[i].first);
        cnt += s.order_of_key(hel[i].first);
    }
    cout << cnt << endl;
}
