...
void solve()
{
    ...
    vector<vector<int>> bitcnt(n + 1);
    for (int i = 0; i < anssize; i++)
    {
        int bits = __builtin_popcountll(i);
        bitcnt[bits].push_back(i);
    }
    ans[0] = anssize - 1;
    int lookingfor = n - 1;
    int currentadd = n;
    ...
    for (int i = 1; i < ans.size(); i++)
    {
        ...
        used[ans[i - 1]] = 1;
        int prebit = __builtin_popcountll(ans[i - 1]);
        if (lookingfor < prebit)
        {
            // look for lookingfor bit number
            int found = 0;
            for (int j = 0; j < bitcnt[lookingfor].size(); j++)
            {
                if (used[bitcnt[lookingfor][j]])
                {
                    continue;
                }
                int common = __builtin_popcountll((ans[i - 1] & bitcnt[lookingfor][j]));
                if (common == lookingfor)
                {
                    found = 1;
                    ans[i] = bitcnt[lookingfor][j];
                    currentadd = lookingfor;
                    break;
                }
            }
            if (found == 0)
        ...
        {
            // look for lookingfor+1bit number
            int found = 0;
            for (int j = 0; j < bitcnt[lookingfor + 1].size(); j++)
            {
                if (used[bitcnt[lookingfor + 1][j]])
                {
                    continue;
                }
                int common = __builtin_popcountll((ans[i - 1] & bitcnt[lookingfor + 1][j]));
                if (common == lookingfor)
                {
                    found = 1;
                    ans[i] = bitcnt[lookingfor + 1][j];
                    currentadd = lookingfor;
                    break;
                }
            }
