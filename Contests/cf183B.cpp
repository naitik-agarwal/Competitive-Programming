...
void solve()
{
    ...
    string s;
    cin >> s;
    int nooftwo = count(all(s), '2');
    int noofone = count(all(s), '1');
    int noofzero = count(all(s), '0');
    int totremtop = noofzero + nooftwo;
    int totrembot = noofone + nooftwo;
    int confremtop = noofzero;
    int confrembot = noofone;
    int l = 0;
    int r = n - 1;
    int remtop = totremtop - confremtop;
    int rembot = totrembot - confrembot;
    vector<char> ans(n, '+');
    for (int i = 0; i < confremtop; i++)
    {
        ans[i] = '-';
    }
    l = confremtop;
    while (r >= 0 && confrembot > 0)
    {
        ans[r] = '-';
        confrembot--;
        r--;
