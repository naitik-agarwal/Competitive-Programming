...
void solve() {
  ...
  sort(all(b));
  int l = 0, r = n - 1;
  int ans = 0, wc = 0, fc = 0, jwfc = 0, jfc = 0;
  fc = count(all(b), 0);
  wc = b.size() - fc;
  int totalworkdone = 0;
  int chance = 0;
  while (totalworkdone < m) {
    sort(all(b));
    chance++;
    ans++;
    if (chance % 2 == 1) {
      totalworkdone += (wc + jwfc);
      int prevjwfc = jwfc;
      auto it = lower_bound(all(b), 1);
      while (it != b.end()) {
        (*it)--;
        if (*it == 0) {
          wc--;
      ...
      // now time to remove prevjwfc from array
      it = lower_bound(all(b), 1);
      while (it != b.end() && prevjwfc != 0) {
        if (*it <= prevjwfc) {
          prevjwfc -= *it;
          *it = 0;
          wc--;
