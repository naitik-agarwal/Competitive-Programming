...
void solve()
{
    ...
    int ans = 0;
    int totalarea = 0;
    while (totalarea != 2 * (l + w))
    {
        if (*sarea.begin() <= *prev(scolor.end()))
        {
            totalarea += *sarea.begin(); // total area incremented
            int diff = *prev(scolor.end()) - *sarea.begin();
            sarea.erase(sarea.begin());
            scolor.erase(prev(scolor.end()));
            scolor.insert(diff);
            ans++;
        }
        else
        {
            int area = *sarea.begin();
            int color = *prev(scolor.end());
            totalarea += color;
            int diff = area - color;
            scolor.erase(prev(scolor.end()));
            sarea.erase(sarea.begin());
