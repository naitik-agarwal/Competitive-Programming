...
void solve()
{
    ...
    int chk = 0;
    int ans = 0;
    vector<pair<int, int>> flase_pair;
    vector<pair<int, int>> odd_freq;
    for (auto &it : freq)
    {
        if (it.second % 2 == 1)
        {
            ans++;
            odd_freq.push_back({it.second, it.first});
        }
        else
        {
            chk = 1;
            ...
            {
                flase_pair.push_back({it.second, it.first});
            }
        }
    }
    if (chk == 0)
    ...
    {
        sort(rall(odd_freq));
        int nooffalse = flase_pair.size();
        int ptr = 0;
        for (int i = 0; i < flase_pair.size() && ptr < odd_freq.size(); i++)
        {
            if (odd_freq[ptr].first > 1)
            {
                ans += 2;
                odd_freq[ptr].first -= 2;
                nooffalse--;
            }
            else
            {
