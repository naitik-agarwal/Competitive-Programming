...
void solve() {
  ...
  int noofminusone = count(all(b), -1);
  if (noofminusone + temp.size() - 1 != n && temp.size() != n) {
    no;
    return;
  }
  temp.erase(temp.begin());
  for (int i = 0; i < temp.size(); i++) {
    int id = idx[temp[i]];  // id = 2
    int id2 = idx2[temp[i]];
    int after = n - id - 1;
    int before = id;
    int aftersort = k - after - 1;
    int beforesort = k - before - 1;
    if (id2 != id) {
      if (id2 < id) {
        if (abs(id2 - id) > aftersort) {
          no;
          return;
        }
      } else {
        if (abs(id2 - id) > beforesort) {
          no;
          return;
        }
      }
