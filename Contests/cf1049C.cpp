...
void solve()
{
    ...
    int maxxx=0;
    for(int i=0;i<add.size();i++){
        maxxx=max(maxxx,add[i].second);
    }
    int maxaddcount=(maxxx-1);
    int maxcnt=(sub.back().second-add[0].second)+(2*sub.back().first)-(2*add[0].first);
    ans2=ans1;
    int left=0;
    int right=n-1;
    if(n%2==1){
    ...
    int maxval=INT_MIN;
    while(left<right){
        maxval=max(maxval,((2*a[right]-2*a[left]+right-left)));
        left+=2;
        right-=2;
    }
    if(n%2==1){
        ...
        int r=n-1;
        while(l<r){
            maxval=max(maxval,(2*a[l]-2*a[r]+r-l));
            l+=2;
            r-=2;
        }
    }
    maxval=max(maxval,maxcnt);
    ans2+=(max(maxaddcount,maxval));
