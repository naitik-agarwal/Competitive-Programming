...
void solve()
{
    ...
    { // this is O(n*log(n))
        if (b[0] < a[i])
        {
            int left_dis = a[i] - (*(lower_bound(all(b), a[i]) - 1));
            death_locations[-left_dis].push_back(i);
        }
        if (b.back() > a[i])
        {
            int right_dis = *lower_bound(all(b), a[i]) - a[i];
            death_locations[right_dis].push_back(i);
        }
    }
    int current_pos = 0;
    int alive = n;
