...
void solve() {
  ...
  string final;
  int ptri = 0, ptrf = n - 1;
  while (ptri < n && s[ptri] == '0') {
    ptri++;
  }
  while (ptrf >= 0 && s[ptrf] == '1') {
    ptrf--;
  }
  if (ptri == ptrf) {
    cout << "Bob" << endl;
  ...
  for (int i = ptri; i <= ptrf; i++) {
    final.push_back(s[i]);
  }
  n = final.size();
  vector<int> zeroafterthat(n);
  for (int i = 0; i < n; i++) {
    if (final[i] == '0') {
      continue;
    } else {
  ...
  if (inversions % 2 == 1) {
    ...
    if (count(all(final), '1') % 2 == 1) {
      cout << "Alice" << endl;
      return;
    }
    for (int i = 0; i < zeroafterthat.size(); i++) {
      if (zeroafterthat[i] % 2 == 1) {
        cout << "Alice" << endl;
        return;
      }
    }
