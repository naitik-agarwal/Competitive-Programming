...
vector<int> get_primeFactorization(int x){
    vector<int> res;
    while(x!=1){
        res.push_back(spf[x]);
        x/=spf[x];
...
void solve()
{
    ...
    if(is_prime(n)==true){
        ...
        vector<int> pFactors=get_primeFactorization(n);
        vector<pair<int,int>> factors;
        for(int i=0;i<pFactors.size();i++){
            int count=0;
            int num=pFactors[i];
            while(i<pFactors.size() && pFactors[i]==num){
                count++;
                i++;
            }
            i--;
            factors.push_back({num,count});
        }
        if(factors.size()>2){
            yes;
            int num1=pow(factors[0].first,factors[0].second);
            int num2=pow(factors[1].first,factors[1].second);
            int num3=1;
            for(int i=2;i<factors.size();i++){
                num3*=pow(factors[i].first,factors[i].second);
            }
            cout<<num1<<" "<<num2<<" "<<num3<<endl;
        }
        else if(factors.size()==2){
            if(factors[0].second+factors[1].second==2 || factors[0].second+factors[1].second==3){
                no;
            }
            else{
                yes;
                ...
                int num=1;
                num*=pow(factors[0].first,factors[0].second-1)*pow(factors[1].first,factors[1].second-1);
                cout<<num;
                cout<<endl;
            }
        }
            ...
            if(factors[0].second<6){
                ...
                yes;
                cout<<factors[0].first<<" "<<factors[0].first*factors[0].first<<" ";
