...
void solve()
{
    ...
    vector<int> xrem, yrem;
    for (int i = 0; i < n; i++)
    {
        xrem.push_back(a[i] % x);
        yrem.push_back(a[i] % y);
        mn[{xrem[i], yrem[i]}]++;
    }
    int ans = 0;
    for (int i = 0; i < n; i++)
    {
        int first = xrem[i], second = yrem[i];
        if (first == 0)
        {
            if (mn[{first, second}] > 1)
            {
                ans += mn[{first, second}] * (mn[{first, second}] - 1) / 2;
                mn[{first, second}] = 0;
            }
        }
        else
        {
            if (mn[{x - first, second}] > 0 && (x - first != first))
            {
                ans += mn[{x - first, second}] * mn[{first, second}];
                mn[{x - first, second}] = 0;
                mn[{first, second}] = 0;
            }
            else if (mn[{x - first, second}] > 0)
            {
                ans += mn[{x - first, second}] * (mn[{x - first, second}] - 1) / 2;
                mn[{x - first, second}] = 0;
            }
        }
    }
