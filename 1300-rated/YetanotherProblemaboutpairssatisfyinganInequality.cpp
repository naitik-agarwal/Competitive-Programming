...
void solve()
{
    ...
    for (int j = 1; j < n; j++)
    {
        ...
        {
            auto it = lower_bound(impi.begin(), impi.end(), a[j]);
            ans += distance(impi.begin(), it);
        }
    }
    cout << ans << endl;
}
...
void sol()
{
    ...
    // mark positions p = 1..n where a[p-1] < p
    vector<int> mark(n + 1, 0); // mark[0] unused, use 1-based index
    for (int p = 1; p <= n; ++p)
    {
        if (a[p - 1] < p)
            mark[p] = 1;
    ...
    // build prefix sums: pref[x] = number of marked positions <= x
    vector<int> pref(n + 1, 0);
    for (int x = 1; x <= n; ++x)
        pref[x] = pref[x - 1] + mark[x];

    long long ans = 0;
    // iterate j (1-based position j)
    for (int j = 1; j <= n; ++j)
    {
        if (a[j - 1] < j)
        {
            int val = a[j - 1]; // we want count of marked positions p < val
            if (val - 1 >= 1)
                ans += pref[val - 1];
            // if val <= 1 then no p < val so contribute 0
        }
    }
