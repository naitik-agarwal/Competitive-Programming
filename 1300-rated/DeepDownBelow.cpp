...
void solve()
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        {
            num = max(num, grid[i][j] + 1 - j);
        }
        int final = num + k[i];

        sum.push_back({num, final});
    }
    sort(all(sum));
    int ans = sum[0].first;
    for (int i = 0; i < n - 1; i++)
    {
        if (sum[i + 1].first > sum[i].second)
        {
            ans += (sum[i + 1].first - sum[i].second);
        }
        else
        {
            sum[i + 1].second = sum[i].second + (sum[i + 1].second - sum[i + 1].first);
            continue;
        }
    }
    cout << ans << endl;
