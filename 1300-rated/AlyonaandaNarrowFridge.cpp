...
void solve()
{
    ...
    multiset<int> blocks2 = blocks;
    if (left > 0)
    {
        blocks.insert(left);
    }
    auto last = prev(blocks2.end());
    int num = *last;
    blocks2.erase(last);
    blocks2.insert(num + left);
    int ptr2 = ptr;
    while (ptr < n && blocks.lower_bound(a[ptr]) != blocks.end())
    {
        auto it = blocks.lower_bound(a[ptr]);
        blocks.erase(it);
        ptr++;
    }
    while (ptr2 < n && blocks2.lower_bound(a[ptr2]) != blocks2.end())
    {
        auto it = blocks2.lower_bound(a[ptr2]);
        blocks2.erase(it);
        ptr2++;
    }
    cout << max(ptr, ptr2) << endl;
}
