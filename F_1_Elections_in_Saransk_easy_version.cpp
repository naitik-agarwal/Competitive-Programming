...
int vPrime(int x, int prime) {
  if (x % prime != 0) {
    return 0;
  }
  return 1 + vPrime(x / prime, prime);
}
...
void solve() {
  ...
  vector<int> prms;
  for (int i = 0; i < n; i++) {
    vector<int> temp = primefac(a[i]);
    for (int j = 0; j < temp.size(); j++) {
      prms.push_back(temp[j]);
    }
  }
  sort(all(prms));
  auto it = unique(all(prms));
  prms.erase(it, prms.end());
  int ans = 1;
  for (int i = 0; i < prms.size(); i++) {
    int temp = 0;
    for (int j = 0; j < n; j++) {
      temp = (temp + (vPrime(a[j], prms[i])) % MOD) % MOD;
    }
    temp = (temp + 1) % MOD;
    ans = (ans * temp) % MOD;
  }
