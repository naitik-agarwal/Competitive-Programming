...
bool neww(vector<int> &num, vector<int> &freq, int ptr) {
  if (freq[ptr] % 2 == 0) {
    return true;
  }
  return false;
...
bool func(vector<int> &num, vector<int> &freq, int ptr, int k) {
  if (neww(num, freq, ptr)) {
    return true;
  } else {
    if (ptr == 0) {
      return false;
    } else {
      if (num[ptr] - num[ptr - 1] <= k) {
        return true;
      } else {
        return func(num, freq, ptr - 1, k);
      }
    }
  }
}
