...
void solve()
{
    ...
    else
    {
        if (k == 0)
        {
            cout << (abs(cord[a - 1].first - cord[b - 1].first) + abs(cord[a - 1].second - cord[b - 1].second)) << endl;
            return;
        }
        else
        {
            int ans1 = (abs(cord[a - 1].first - cord[b - 1].first) + abs(cord[a - 1].second - cord[b - 1].second));
            int ans2 = 0;
            int min1 = (abs(cord[a - 1].first - cord[0].first) + abs(cord[a - 1].second - cord[0].second));
            int min2 = (abs(cord[0].first - cord[b - 1].first) + abs(cord[0].second - cord[b - 1].second));
            for (int i = 0; i < k; i++)
            {
                min1 = min(min1, (abs(cord[a - 1].first - cord[i].first) + abs(cord[a - 1].second - cord[i].second)));
            }
            for (int i = 0; i < k; i++)
            {
                min2 = min(min2, (abs(cord[b - 1].first - cord[i].first) + abs(cord[b - 1].second - cord[i].second)));
            }
            ans2 = min1 + min2;
