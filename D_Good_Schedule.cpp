...
void solve() {
  ...
  vector<int> dp(n + 1);
  int current = 0, totalvalid = 0;
  // dp[i] => means no of different segments till current day who wants to see
  // next episode as ith one means both have successfully watched 1,2,...i-1
  // episodes together
  for (int i = 0; i < n; i++) {
    dp[1]++;
    current++;
