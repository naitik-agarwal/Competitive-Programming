...
void solve()
{
    ...
    int ans=0;
    vector<pair<int,int>> a2,b2,c2;
    for(int i=0;i<n;i++){a2.push_back({a[i],i});};
    for(int i=0;i<n;i++){b2.push_back({b[i],i});};
    for(int i=0;i<n;i++){c2.push_back({c[i],i});};
    sort(rall(a2));
    sort(rall(b2));
    sort(rall(c2));
    for(int i=0;i<3;i++){
        for(int j=0;j<3;j++){
            for(int k=0;k<3;k++){
                int indexa=a2[i].second, indexb=b2[j].second, indexc=c2[k].second;
                if(indexa!=indexb && indexa!=indexc && indexc!=indexb){
                    ans=max(ans,a[indexa]+b[indexb]+c[indexc]);
                }
            }
        }
    }
