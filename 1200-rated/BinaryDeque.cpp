...
void solve()
{
    ...
    {
        int remove = sum - s; // itne hatane hai
        // x start se hatao and remove-x last se
        vector<int> prefixsums, prefixsumb;
        int start = 0, back = 0;
        for (int i = 0; i < n; i++)
        {
            start += a[i];
            prefixsums.push_back(start);
        }
        for (int i = n - 1; i >= 0; i--)
        {
            back += a[i];
            prefixsumb.push_back(back);
        }
        int ans = min(lower_bound(all(prefixsumb), remove) - prefixsumb.begin() + 1, lower_bound(all(prefixsums), remove) - prefixsums.begin() + 1);
        for (int i = 1; i < remove; i++)
        {
            // start se i hatao
            int cnt = 0;
            cnt += (lower_bound(all(prefixsums), i) - prefixsums.begin()) + 1;
            cnt += (lower_bound(all(prefixsumb), remove - i) - prefixsumb.begin() + 1);
            ans = min(ans, cnt);
        }
        cout << ans << endl;
    }
