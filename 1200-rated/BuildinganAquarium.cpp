...
void solve()
{
    ...
    vector<pair<int, int>> totalwaterreq;
    sort(all(a));
    int len = 0;
    for (auto it = mp.begin(); it != prev(mp.end()); ++it)
    {
        ...
        auto temp = it;
        temp++;
        totalwaterreq.push_back({len * (temp->first - it->first), temp->first});
    }
    if (totalwaterreq.empty() == 1)
    {
        cout << a[0] + y / n << endl;
        return;
    }
    int x = totalwaterreq[0].first;
    for (int i = 1; i < totalwaterreq.size(); i++)
    {
        x += totalwaterreq[i].first;
        totalwaterreq[i].first = x;
    }
    if (y >= totalwaterreq.back().first)
    {
        int ans = totalwaterreq.back().second;
        y -= totalwaterreq.back().first;
        ans += y / n;
        cout << ans << endl;
    }
    else
    {
        ...
        int waterleft = y;
        int ln = 0;
        for (int i = 0; i < totalwaterreq.size(); i++)
        {
            if (totalwaterreq[i].first <= y)
            {
                ans = totalwaterreq[i].second;
            }
            else
            {
                if (i != 0)
                {
                    waterleft -= totalwaterreq[i - 1].first;
                }
                int ch = totalwaterreq[i].second;
                ln = lower_bound(all(a), ch) - a.begin();
                if (ln != 0)
                {
                    waterleft /= ln;
                }
