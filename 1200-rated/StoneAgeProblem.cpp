...
void solve()
{
    ...
    vector<int> lastFirstQuerry(n), lastFirstQuerryValue = a;
    int lastSecondQuerry = -1, lastSecondQuerryValue = -1;
    for (int i = 1; i <= q; i++)
    {
        int x;
        cin >> x;
        ...
        {
            int pos, val;
            cin >> pos >> val; // pos given is 1-based indexing
            if (lastSecondQuerry > lastFirstQuerry[pos - 1])
            {
                sum -= lastSecondQuerryValue;
            }
            else
            {
                sum -= lastFirstQuerryValue[pos - 1];
            }
            sum += val;
            cout << sum << endl;
            lastFirstQuerry[pos - 1] = i;
            lastFirstQuerryValue[pos - 1] = val;
        }
        else
        {
            int val;
