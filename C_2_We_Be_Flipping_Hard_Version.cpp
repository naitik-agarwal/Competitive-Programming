...
vector<int> func(vector<int> &a, int ptrr) {
  vector<int> sol;
  int ans = 0;
  int ptr = ptrr - 1;
  while (ptr >= 0 && a[ptr] < 0) {
...
void solve() {
  ...
  // base case
  if (*max_element(all(a)) < 0 || *min_element(all(a)) > 0) {
    cout << 0 << endl << endl;
    return;
  }
  // main code
  ...
  while (last >= 0) {
    if (a[last] < 0) {
      ...
      int currsum = sum - 2 * a[last] + 2 * negsum;
      if (currsum > curr) {
        curr = currsum;
        mainptr = last;
      }
  ...
  if (summ >= curr || mainptr == -1) {
    cout << 0 << endl << endl;
    return;
  }
  vector<int> sol = func(a, mainptr);
  int cnt = sol.back();
  sol.pop_back();
  cout << cnt + 1 << endl;
