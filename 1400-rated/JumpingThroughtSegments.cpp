...
void solve()
{
    ...
    while (left <= right)
    {
        ...
        // now check if its possible to go by mid
        int lseg = 0, rseg = 0;
        int ptr = 0;
        int possible = 1;
        while (ptr < n)
        {
            lseg -= mid;
            rseg += mid;
            if (d[ptr].first > rseg || d[ptr].second < lseg)
            {
                possible = 0;
                break;
            }
            ...
            {
                if (lseg <= d[ptr].first && rseg >= d[ptr].second)
                {
                    lseg = d[ptr].first;
                    rseg = d[ptr].second;
                }
                else if (lseg < d[ptr].first)
                {
                    lseg = d[ptr].first;
                }
                else if (rseg > d[ptr].second)
                {
                    rseg = d[ptr].second;
                }
            }
