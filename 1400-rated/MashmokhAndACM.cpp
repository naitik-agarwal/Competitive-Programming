...
void solve()
{
    int n,k;
    cin>>n>>k;
    // Create a DP table what will store the number of factor chains of length i ending with j
    vector<vector<int>> dp_table(k+1,vector<int> (n+1,0));
    // Base Case
    for(int j=1;j<=n;j++){
        dp_table[1][j]=1;
    }
    // Build up the table from down to top approach i.e. from length i=2 to k
    for(int i=2;i<=k;i++){
        // for each possible previous number j in chain of length i-1
        for(int j=1;j<=n;j++){
            // The number of chains of length i-1 ending with j is dp_table[i-1][j]
            // This value contributes to all chain of length i that ends in a multiple of j
            for(int m=j;m<=n;m+=j){
                dp_table[i][m]=(dp_table[i][m]+dp_table[i-1][j])%MOD;
            }
        }
    }
    // Final ans is sum of all chains of length k
    int ans=0;
    for(int j=1;j<=n;j++){
        ans=(ans+dp_table[k][j])%MOD;
    }
    cout<<ans<<endl;
}
