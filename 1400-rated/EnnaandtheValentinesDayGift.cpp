...
void solve()
{
    ...
    {
        cin >> a[i].first;
        totalnoofdig += floor(log10(a[i].first)) + 1;
        int j = 0;
        for (int k = 1; k <= 10; k++)
        {
            if (a[i].first % (poweroften(k)) == 0)
            {
                continue;
            }
            else
        ...
        a[i].second = j;
        lastzero.push_back(j);
    }
    int reqdig = m + 1;
    sort(rall(lastzero));
    int chance = 0; // anna's chance
    int l = 0;
    while (l < lastzero.size())
    {
        if (chance == 0)
        {
            totalnoofdig -= lastzero[l];
            l++;
            chance = 1;
        }
        else
