...
void solve() {
  ...
  // first eliminate cases which are gauranteed to be failed
  int minx = min(x1, x2), miny = min(y1, y2);
  int maxx = max(x1, x2), maxy = max(y1, y2);
  if (maxx - minx - a > 0 && maxx - minx - a < a) {
    if (maxy - miny - b < b) {
      no;
    } else {
      if ((maxy - miny - b) % b != 0) {
  ...
  } else if (maxy - miny - b > 0 && maxy - miny - b < b) {
    if (maxx - minx - a < a) {
      no;
    } else {
      if ((maxx - minx - a) % a != 0) {
    ...
    if ((maxx - minx - a > 0 && (maxx - minx - a) % a != 0) ||
        (maxy - miny - b > 0 && (maxy - miny - b) % b != 0)) {
      no;
    } else {
      yes;
    }
