...
void solve() {
    ...
    // 1. Parse input, reverse, and keep only unique elements per blog
    for (int i = 0; i < n; i++) {
        int l;
        cin >> l; // NOTE: The first number is the length! Don't treat it as a user ID.
        vector<int> a(l);
        for (int j = 0; j < l; j++) {
            cin >> a[j];
        }
        ...
        unordered_set<int> seen_in_blog;
        for (int x : a) {
            if (seen_in_blog.insert(x).second) {
                U[i].push_back(x);
            }
        }
    }
    ...
    vector<bool> seen_global(1000005, false); // Tracks elements already in final Q
    vector<int> final_Q;
    
    // 2. Greedily pick the blog that contributes the lexicographically smallest sequence
    for (int step = 0; step < n; step++) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            // Calculate what this blog would contribute right now
            vector<int> current_seq;
            for (int x : U[i]) {
                if (!seen_global[x]) {
                    current_seq.push_back(x);
                }
            }
            
            // If it's the first available blog we checked, or strictly smaller than our best
            if (best_blog == -1 || current_seq < best_seq) {
                best_seq = current_seq;
                best_blog = i;
            }
        }
        
        // 3. Lock in the best blog
        used_blog[best_blog] = true;
        for (int x : best_seq) {
            final_Q.push_back(x);
            seen_global[x] = true;
        }
