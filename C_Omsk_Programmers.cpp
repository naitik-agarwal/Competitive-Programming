...
int func(int a, int b, int x) {
  ...
  if (b > a) {
    return min(1 + func(a + 1, b, x), 1 + func(a, b / x, x));
  } else {
    return min(1 + func(a / x, b, x), 1 + func(a, b + 1, x));
  }
}

// cant apply dp since a,b are upto 1e9

int fuc(int a, int b, int x) {
  if (a == b) {
    return 0;
  }
  if (a > b) {
    return min(a - b, 1 + fuc(a / x, b, x));
  } else {
    return min(b - a, 1 + fuc(a, b / x, x));
  }
}
